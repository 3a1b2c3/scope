...
def plugins():
    ...
    if not plugin_list:
        click.echo("1No plugins installed.")
        return
...
def pipelines():
    ...
    if not all_pipelines:
        click.echo("1No pipelines available.")
        return

    click.echo(f"{len(all_pipelines)} pipeline(s)1 available:\n")
...
def install(packages, upgrade, editable, force_reinstall, no_cache_dir, pre, force):
    ...
    print("packages_to_validate1", packages)
